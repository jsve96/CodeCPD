...
Dataset_path = os.path.join(current_directory,"datasets","run_log.json")
...
annotations = META_ANNOTATIONS['run_log']
...
np.random.seed(0)
SWATCH = p_Detector(mat,K=8,eps=1.2,kappa=2,mu=7,L=100,p=2,fimp=True)
cps = SWATCH.run()
print(annotations)
print(cps)
...
fig.savefig(os.path.join(current_directory,"appendix","runlog_plot.pdf"))
