...
dir_corruptions = os.path.join(dir_path,'rotate')

TARGET_ZIP = 'MNIST-C3-compressed.zip'
...
oracle_annotations = {"MNIST_C3_" + str(i): anno for i, anno in enumerate(annoationslist_corr)}
...
for i, file in enumerate(datasetslist_merged):
    ...
    with open(os.path.join(dir_path,"MNIST_C3_"+str(i)+".json"),"w") as file:
