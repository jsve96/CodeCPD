...
if (isZip){
    ...
    ECP_ORACLE_DIR <- file.path(file_dir,"oracle_ecp")
    if (!dir.exists(ECP_ORACLE_DIR)){
             dir.create(ECP_ORACLE_DIR)
             print("create new dir")
    }
     ...
     for (i in 1:length(out_long)){
        if(!is.null(out_long[i]$SETTING)){
            ...
            file_name = file.path(ECP_ORACLE_DIR,paste(temp_file$SETTING,"json",sep="."))
