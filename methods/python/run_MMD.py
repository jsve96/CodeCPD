...
for name in DATASETS:
    ...
    if isZIP(tmp_path):
        with zipfile.ZipFile(os.path.join(tmp_path+'.zip'),"r") as archive:
            ...
            for file in FILES[4:5]:
                ...
                for ind in range(len(results['F1'])):
                    out_dict = {'Setting':{'name':fname}, 'info':{'Method':'MMD','params':results['parameter'][ind],'cp':results['cp_id'][ind],'F1':results['F1'][ind],'Covering':results['covering'][ind],'runtime':results['runtime'][ind]}}
                    print(out_dict)
        ...
        SWATCH = MMD_Detector(mat,K=9,eps=1.5,kappa=3,mu=8,L=100)
        start = time.time()
        results = SWATCH.grid_search(GRID,ground_truth)
        #results = SWATCH.solo_grid(GRID,ground_truth)
        end = time.time()
        ...
        for ind in range(len(results['F1'])):
            out_dict = {'Setting':{'name':name}, 'info':{'Method':'MMD','params':results['parameter'][ind],'cp':results['cp_id'][ind],'F1':results['F1'][ind],'Covering':results['covering'][ind],'runtime':results['runtime'][ind]}}
