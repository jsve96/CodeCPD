    ...
    if isZIP(tmp_path):
        with zipfile.ZipFile(os.path.join(tmp_path+'.zip'),"r") as archive:
            ...
            for file in FILES[4:5]:
                ...
                SWATCH = p_Detector(mat,K=4,eps=1.1,kappa=5,mu=8,L=100,p=2,delta=0.1)
                start = time.time()
                cps = SWATCH.run()
                end = time.time()
                print(end-start)
                ...
                for ind in range(len(results['F1'])):
                    out_dict = {'Setting':{'name':fname}, 'info':{'Method':'SWD10','params':results['parameter'][ind],'cp':results['cp_id'][ind],'F1':results['F1'][ind],'Covering':results['covering'][ind],'runtime':results['runtime'][ind]}}
                    print(out_dict)
                    cur_out_dir = os.path.join(RESULTS_PATH,name,fname)
                    if not os.path.exists(cur_out_dir):
                        os.mkdir(os.path.join(cur_out_dir))

                    SWD_dir = os.path.join(cur_out_dir,"oracle_SWD10")
        ...
        SWATCH = p_Detector(mat,K=9,eps=1.5,kappa=3,mu=8,L=100,p=2,delta=0.1)
        start = time.time()
        results = SWATCH.grid_search(GRID,ground_truth)
        #results = SWATCH.solo_grid(GRID,ground_truth)
        end = time.time()
        ...
        for ind in range(len(results['F1'])):
            out_dict = {'Setting':{'name':name}, 'info':{'Method':'SWD10','params':results['parameter'][ind],'cp':results['cp_id'][ind],'F1':results['F1'][ind],'Covering':results['covering'][ind],'runtime':results['runtime'][ind]}}
